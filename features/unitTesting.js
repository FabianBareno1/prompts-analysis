...
export function renderCoveragePerModule(data, chart, width, height) {
  // Group rows by module, filtering out rows without a valid module
  const filteredData = data.filter(d => d.Module || d.module);
  const grouped = d3.group(filteredData, d => d.Module || d.module);
